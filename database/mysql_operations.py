...
class MySQLMealOperations:
    """MySQL operations for meal-related database functions"""
    ...
    def _update_daily_summary(self, cursor, user_id: int, date_obj: date, calories: float):
        ...
        cursor.execute(
            'SELECT total_calories, meal_count FROM daily_summaries WHERE user_id = %s AND date = %s',
            (user_id, date_obj)
        )
        ...
        if result:
            ...
            cursor.execute('''
                ...
                SET total_calories = %s, meal_count = %s, updated_at = %s
                WHERE user_id = %s AND date = %s
            ''', (new_total, new_count, now, user_id, date_obj))
        else:
            # Create new summary
            cursor.execute('''
                ...
                VALUES (%s, %s, %s, %s, %s, %s)
            ''', (user_id, date_obj, calories, 1, now, now))
    ...
    def get_daily_summary(self, user_id: int, date_str: str = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cursor = conn.cursor(dictionary=True)
            cursor.execute(
                'SELECT * FROM daily_summaries WHERE user_id = %s AND date = %s',
                (user_id, date_str)
            )
            row = cursor.fetchone()
            cursor.close()
    ...
    def get_user_stats(self, user_id: int, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            total_calories = sum(float(s['total_calories']) for s in summaries)
    ...
    def clear_user_data_by_date(self, user_id: int, date_str: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                'DELETE FROM meals WHERE user_id = %s AND date = %s',
                (user_id, date_str)
            )
    ...
    def clear_user_data_range(self, user_id: int, start_date: str, end_date: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                'DELETE FROM meals WHERE user_id = %s AND date >= %s AND date <= %s',
            ...
            cursor.execute(
                'DELETE FROM daily_summaries WHERE user_id = %s AND date >= %s AND date <= %s',
                (user_id, start_date, end_date)
            )
    ...
    def clear_all_user_data(self, user_id: int) -> bool:
        ...
        try:
            ...
            cursor.execute('DELETE FROM daily_summaries WHERE user_id = %s', (user_id,))
            summaries_deleted = cursor.rowcount
            
            # Delete any pending meals
            cursor.execute('DELETE FROM pending_meals WHERE user_id = %s', (user_id,))
    ...
    def get_user_data_summary(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Count total meals
            cursor.execute('SELECT COUNT(*) FROM meals WHERE user_id = %s', (user_id,))
            total_meals = cursor.fetchone()[0]
            
            # Count days with data
            cursor.execute('SELECT COUNT(DISTINCT date) FROM meals WHERE user_id = %s', (user_id,))
            days_with_data = cursor.fetchone()[0]
            
            # Get date range
            cursor.execute(
                'SELECT MIN(date), MAX(date) FROM meals WHERE user_id = %s',
                (user_id,)
            )
            date_range = cursor.fetchone()
            
            # Get total calories
            cursor.execute('SELECT SUM(calories) FROM meals WHERE user_id = %s', (user_id,))
            ...
            return {
                ...
                'first_meal_date': str(date_range[0]) if date_range[0] else None,
                'last_meal_date': str(date_range[1]) if date_range[1] else None,
